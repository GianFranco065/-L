...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 🔧 CORRECCIÓN CRÍTICA: Forzar mostrar login después de inicialización
    setTimeout(() => {
        const loading = document.getElementById('loading');
        const loginScreen = document.getElementById('loginScreen');
        
        if (loading && loginScreen) {
    ...
    }, 2000); // Dar tiempo para que se complete la inicialización
});
...
function checkSavedSession() {
    ...
    if (savedUser) {
        ...
        // 🔧 CORRECCIÓN: No llamar showLogin aquí, se llamará desde el setTimeout
...
function showLogin() {
    // 🔧 CORRECCIÓN CRÍTICA: Asegurar que se oculte el loading
    const loading = document.getElementById('loading');
    if (loading) {
        loading.style.display = 'none';
    }
...
function showMainApp() {
    // 🔧 CORRECTION: Hide loading and login screen
    const loading = document.getElementById('loading');
    if (loading) {
        loading.style.display = 'none';
    }
...
function createMantenimientosChart(data) {
    try {
        ...
        mantenimientosChart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: ['Preventivos', 'Correctivos', 'Predictivos'],
                datasets: [{
...
function renderDataTable() {
    ...
    if (totalRecords === 0) {
        dataTableContainer.innerHTML = '<div class="no-data">No hay registros para mostrar</div>';
        return;
    }
    ...
    const pageData = filteredData.slice(startIndex + 1, endIndex + 1); // +1 para saltar headers
    ...
    const tableHTML = `
        <table class="data-table">
            <thead>
                <tr>
                    ${headers.map((header, index) => `
                        <th onclick="sortTable(${index})" class="sortable">
                            ${header.replace(/_/g, ' ')}
            ...
            <tbody>
                ${totalRecords === 0 ? 
                    `<tr><td colspan="${headers.length + 1}">No hay registros para mostrar</td></tr>` :
...
function generateItemsField(header, fieldId, value) {
    const itemType = header === 'FALLAS' ? 'fallas' : 'piezas';
...
function generateDropdownField(header, fieldId, value) {
    ...
    if (header.toUpperCase() === 'MAQUINARIA' || header.toUpperCase() === 'MAQUINA_ORIGEN' || header.toUpperCase() === 'MAQUINA_DESTINO') {
    ...
    } else if (header.toUpperCase() === 'PERSONAL') {
        options = dropdownData.personal || [];
        ...
        options = dropdownData[config.source?.toLowerCase()] || config.options || [];
    }
...
// ===== ANIMACIONES CSS (inyectadas via JavaScript) =====
...
// ===== FUNCIONES DE NAVEGACIÓN FALTANTES =====
...
function closeMultipleModal() {
    ...
    console.log('closeMultipleModal llamada');
}
...
function backToDashboard() {
    try {
        ...
        showError('Error regresando al dashboard');
    }
}
...
function refreshModule() {
    try {
        if (currentModule) {
            console.log('🔄 Refrescando módulo:', currentModule);
            loadModule(currentModule);
        } else {
            showError('No hay módulo activo para refrescar');
        }
    } catch (error) {
        console.error('❌ Error refrescando módulo:', error);
        showError('Error refrescando módulo');
    }
}

// ===== FUNCIONES DE PAGINACIÓN FALTANTES =====
function prevPage() {
    if (currentPage > 1) {
        currentPage--;
        renderDataTable();
...
// ===== FUNCIONES DE FORMULARIO FALTANTES =====
function generateItemsField(header, fieldId, value) {
            ...
            <div class="items-container">
                <div class="item-input-section">
                    <input 
                        ...
                        placeholder="Ingrese ${itemType === 'fallas' ? 'falla' : 'pieza'}..."
                    >
                    <button type="button" onclick="addItem('${fieldId}', '${itemType}')" class="btn-add-item">
                        <i class="fas fa-plus"></i> Agregar
                    </button>
...
function getDropdownOptions(header) {
    ...
    if (upperHeader === 'MAQUINARIA' || upperHeader === 'MAQUINA_ORIGEN' || upperHeader === 'MAQUINA_DESTINO') {
        return dropdownData.maquinarias || [];
    }
    
    if (upperHeader === 'PERSONAL' || upperHeader === 'PERSONAL_ASIGNADO') {
        return dropdownData.personal || [];
    }
    
    if (upperHeader === 'ESTADO') {
        if (currentModule === 'Maquinarias') {
            return ['OPERATIVO', 'INOPERATIVO', 'STANDBY'];
        }
        if (currentModule === 'Programacion_Mantenimiento') {
            return ['PENDIENTE', 'PROXIMO', 'URGENTE', 'REALIZADO'];
        }
        return ['ACTIVO', 'INACTIVO', 'PENDIENTE', 'REALIZADO'];
    }
    
    if (upperHeader.includes('TIPO_MANTENIMIENTO')) {
        return ['PREVENTIVO', 'CORRECTIVO', 'PREDICTIVO'];
    }
    
    if (upperHeader.includes('PRIORIDAD')) {
